...
def index():
    return render_template('base/base.html', list_recommend_book_new=utils.recommend_bookNew(),
                           list_recommend_book=utils.recommend_book(), list_best_sale_book=utils.best_sale_book(),
                           list_book_image=utils.load_book_image(), list_book_category=utils.get_book_category())
...
def verify():
    if request.method == "POST":
        ...
        if verification.ok():
            ...
            db.session.commit()
            return Response(
                "<h1>Xác thực thành công, bạn vui lòng đăng nhập lại với email đã đăng kí và mật khẩu: " + random_str + "</h1>")
    ...
    try:
        ...
        for item in list_item:
            if str(item.id_book) == id_book:
                ...
                db.session.commit()
        if flag == 0:
            newitem = CartItem(id_cart=id_cart, id_book=id_book, quantity=quantity, price=price,
...
def payment():
    ...
    return render_template('payment.html', id_cart=id_cart, list_item=list_item, total_amount=total_amount,
                           total_quantity=total_quantity, list_book=utils.load_Book(),
                           list_book_category=utils.get_book_category(), list_image=utils.get_all_image())
...
def order_history():
    return render_template('order_history.html', list_book_category=utils.get_book_category(),
                           list_bill=utils.get_list_bill(current_user.id), list_bill_detail=BillDetail.query.all())
...
def pay():
    try:
        ...
        bill = Bill(id_user=current_user.id, address_delivery=address_receiver, phone=phone_number_receiver,
                    name_delivery=name_receiver, total_price=total_amount)
        db.session.add(bill)
...
def confirm_pay():
    ...
    return render_template('confirm__pay.html', id_cart=id_cart, list_item=list_item, total_amount=total_amount,
                           total_quantity=total_quantity, list_book=utils.load_Book(),
                           list_book_category=utils.get_book_category(), list_image=utils.get_all_image(), dola=dola)
...
def search():
    ...
    n = len(listBook)
    return render_template('search.html', listBook=listBook, list_book_category=utils.get_book_category(), len=n,
                           listImage=utils.loadImageByListIdBook(listBook))
...
def searchCategory(id_category):
    ...
    n = len(listcate)
    return render_template('search.html', listBook=listcate, len=n, listImage=utils.loadImageByListIdBook(listcate),
                           list_book_category=utils.get_book_category(), list_book=utils.load_Book(),
                           list_book_image=utils.load_book_image())
...
def load_detail_book_by_id(id_book):
    ...
    return render_template('single.html', book=book, list_image=utils.get_image_by_id_book(id_book),
                           list_book_category=utils.get_book_category())
